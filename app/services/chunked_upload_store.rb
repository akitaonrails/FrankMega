class ChunkedUploadStore
  ...
  def write_chunk!(index:, upload:)
    ...
    raise Error, I18n.t("uploads.chunked.errors.chunk_too_large") if upload.tempfile.size > self.class.chunk_size

    upload.tempfile.rewind
    FileUtils.mkdir_p(chunks_path)
    File.binwrite(chunk_path(index), upload.tempfile.read)
  rescue ArgumentError, TypeError
  ...
  def complete!(user:)
    ...
    content_type = File.open(merged_path, "rb") { |file| Marcel::MimeType.for(file, name: filename) }
